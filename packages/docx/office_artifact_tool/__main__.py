...
def main(argv=None):
 ...
 i=sub.add_parser('inspect');i.add_argument('--source',required=True);i.add_argument('--view',default='full',choices=['full','search','inventory']);i.add_argument('--query')
 ...
 t=sub.add_parser('fill-template');t.add_argument('--source',required=True);t.add_argument('--values',required=True);t.add_argument('--output',required=True)
 ...
 elif args.action=='fill-template':result=tool.fill_template(args.source,load(args.values),args.output)
