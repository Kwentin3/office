...
import copy,json,posixpath,zipfile
from pathlib import Path
from typing import Any
from lxml import etree
from ..core.errors import ArtifactError
from ..core.hashes import file_sha256
from .inspect import W,NS,_stories,_text,_style,_id
...
def _resolve(parts,source):
 ...
 for part,meta in stories.items():
...
def _replace(p,old,new):
 ...
 result=joined.replace(old,new,1);nodes[0].text=result
...
def mutate(source:Path,plan:dict[str,Any],candidate:Path)->dict:
 ...
 for op in plan['operations']:
  ...
  if kind=='replace_text':
   ...
   if target['kind']!='row' or set(op)!={'type','target_id'}:raise ArtifactError('validation_failure','invalid row deletion')
