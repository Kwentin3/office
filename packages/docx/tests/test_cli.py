...
PY=sys.executable;ROOT=Path(__file__).resolve().parents[1]
 ...
 def test_json_cli_create_inspect_plan_apply_validate(self):
  with tempfile.TemporaryDirectory() as d:
   ...
   def run(*args):
    ...
    p=subprocess.run([PY,'-m','office_artifact_tool','--workspace',str(d/'work'),*map(str,args)],cwd=ROOT,env=env,text=True,capture_output=True);self.assertEqual(p.returncode,0,p.stderr);return json.loads(p.stdout)
