...
class DistributionVerifierTests(unittest.TestCase):
    ...
    def write_archives(
        ...
        *,
        ...
        wheel_name: str = "kwentin_office-0.5.0-py3-none-any.whl",
        sdist_name: str = "kwentin_office-0.5.0.tar.gz",
    ) -> None:
        ...
        with tarfile.open(sdist, "w:gz") as archive:
            for suffix in sorted(SDIST_MEMBERS - ({omit_sdist} if omit_sdist else set())):
                payload = b"x"
                info = tarfile.TarInfo(f"kwentin_office-0.5.0/{suffix}")
                info.size = len(payload)
                archive.addfile(info, BytesIO(payload))
            if extra_sdist:
                payload = b"x"
                info = tarfile.TarInfo(f"kwentin_office-0.5.0/{extra_sdist}")
                info.size = len(payload)
                archive.addfile(info, BytesIO(payload))
    ...
    def test_metadata_and_top_level_contents_are_closed(self) -> None:
        for kwargs in (
            {"metadata": VALID_METADATA.replace(b"Version: 0.5.0", b"Version: 9.9.9")},
    ...
    def test_artifact_filenames_are_bound_to_name_version_and_tag(self) -> None:
        for kwargs in (
            {"wheel_name": "renamed-0.5.0-py3-none-any.whl"},
            {"wheel_name": "kwentin_office-0.5.0-cp312-cp312-manylinux_2_17_x86_64.whl"},
            {"sdist_name": "renamed-0.5.0.tar.gz"},
