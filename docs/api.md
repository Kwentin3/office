...
`render_docx_preview` and `render_xlsx_preview` atomically publish closed `review.json` packets and bounded HTML artifacts. DOCX uses a closed `professional-a4/v2` presentation profile for both the styled page-layout proxy and final DOCX; its revision binds the complete validated semantic model plus presentation ID. Only managed paragraph styles and `Table Grid` are admitted so preview does not silently flatten unsupported styling. XLSX remains a structural grid whose revision binds its validated model. Existing-review replacement uses Linux `renameat2(RENAME_EXCHANGE)` so readers never observe a missing publication directory; an unavailable atomic-exchange primitive refuses the replacement and preserves the prior review. Artifact SHA-256 values bind the published HTML bytes. HTML is escaped and script-free. DOCX line wrapping/pagination and Excel application rendering remain explicitly unclaimed; formula text is never evaluated by preview.
